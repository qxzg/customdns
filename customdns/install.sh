...
install_now(){
	# default value
	local TITLE="自定义DNS"
	local DESCR="customdns"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/customdns_config.sh" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		sh /koolshare/scripts/customdns_config.sh stop >/dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*customdns_config*" | xargs rm -rf >/dev/null 2>&1
	...
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/customdns_config.sh" ];then
		...
		sh /koolshare/scripts/customdns_config.sh start
	fi
